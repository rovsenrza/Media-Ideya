...
class CAPTCHA{
	...
	function __construct(){
		...
		$jpeg_quality = 90;
		$fonts=array();
		$fontsdir_absolute = ROOT_DIR.'/'.$fontsdir;
		...
		for($x=0;$x<$width;$x++){
			for($y=0;$y<$height;$y++){
				...
				if($color==255 && $color_x==255 && $color_y==255 && $color_xy==255){
					...
					$newred=intval($newcolor0*$foreground_color[0]+$newcolor*$background_color[0]);
					$newgreen=intval($newcolor0*$foreground_color[1]+$newcolor*$background_color[1]);
					$newblue=intval($newcolor0*$foreground_color[2]+$newcolor*$background_color[2]);
				}
