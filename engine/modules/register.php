...
$canonical = $_SERVER['SCRIPT_NAME']."?do=register";
...
function check_reg($name, $email, $password1, $password2, $sec_code = 1, $sec_code_session = 1) {
	...
	if( !is_valid_email($email) ) $stop .= $lang['reg_err_6'];
	...
	if( !$stop ) {
		...
		$row = $db->super_query( "SELECT COUNT(*) as count FROM " . USERPREFIX . "_users WHERE email = '". $db->safesql($email)."' OR LOWER(name) REGEXP '^{$search_name}$' OR name = '{$name}'" );
...
if( isset( $_POST['submit_reg'] ) AND !$stopregistration ) {
	...
	if ( $config['sec_addnews'] ) {
		...
		if ($sfs->is_spammer( $args )) {
			...
			$db->query( "UPDATE " . PREFIX . "_spam_log SET is_spammer='1', email='" . $db->safesql($email) . "' WHERE ip = '{$_IP}'" );
			$stopregistration = TRUE;
			$reg_error .= $lang['reg_err_35'];

		} else {

			$db->query( "UPDATE " . PREFIX . "_spam_log SET email='" . $db->safesql($email) . "' WHERE ip = '{$_IP}'" );
	...
	if( $config['registration_rules'] and !isset($_SESSION['dle_rules_accept']) ) {
		...
		if( $config['allow_sec_code'] ) {
			...
			if ( $config['allow_recaptcha'] ) {
				...
				$tpl->set( '{reg_code}', "<a onclick=\"reload(); return false;\" href=\"#\" title=\"{$lang['reload_code']}\"><span id=\"dle-captcha\"><img src=\"{$path['path']}index.php?controller=antibot\" alt=\"{$lang['reload_code']}\" width=\"160\" height=\"80\"></span></a>" );
...
if( isset( $_POST['submit_val'] ) AND !$stopregistration ) {
				...
				if ( !$thumb->error) {
					...
					if ( $foto_name AND !$thumb->error) {
						...
						if ( $driver AND !DLEFiles::$remote_error ) {
							...
							if (strpos($config['http_home_url'], "//") === 0) $avatar_url = isSSL() ? "https:" . $config['http_home_url'] : "http:" . $config['http_home_url'];
							elseif (strpos($config['http_home_url'], "/") === 0) $avatar_url = isSSL() ? "https://" . $_SERVER['HTTP_HOST'] . $config['http_home_url'] : "http://" . $_SERVER['HTTP_HOST'] . $config['http_home_url'];
	...
	if( !$stop) {
		...
		$filecontents = DLEUserXFields::Parse();
...
if( $doaction == "validating" AND !$stopregistration AND !isset($_POST['submit_val']) ) {
	...
	$email = sanitize_email($user_arr[1]);
	...
	if( $reg_error ) {
		...
		if( ($_REQUEST['step'] != 2) and $config['registration_type'] ) {
			...
			$db->query( "INSERT INTO " . USERPREFIX . "_users (name, password, email, reg_date, lastdate, user_group, info, signature, favorites, xfields, logged_ip, hash) VALUES ('{$name}', '{$regpassword}', '". $db->safesql($email)."', '{$add_time}', '{$add_time}', '{$config['reg_group']}', '', '', '', '', '{$_IP}', '{$hash}')" );
			$id = $db->insert_id();
...
if( $doaction == "validating" AND !$stopregistration ) {
	...
	$xfields = DLEUserXFields::FieldsList(null, 'site');
