...
{$lang['enter_n_pass_1']}
<br>{status}<br>
<label for="static_password">{$lang['enter_n_pass_2']}</label><input type="password" name="static_password" id="static_password" style="width:100%; max-width:200px;">
<br><br>
...
HTML;
		...
		if( $config['allow_alt_url'] AND $static_result['name'] == "dle-rules-page" ) {
			$static_result['name'] = "rules";
		}

		$full_link = DLEUrl::BuildUrl('static', ['page' => $static_result['name']]);
		...
		if( $static_result['allow_template'] OR $view_template == "print" ) {
			...
			if( $view_template == "print" ) {
				...
				if ($config['seo_control'] AND isset($_GET['news_page']) AND ($_GET['news_page'] < 2 OR $_GET['news_page'] > $anzahl_seiten )) {
						...
						$re_url = DLEUrl::BuildUrl('static', ['page' => $static_result['name']]);
				...
				if( $anzahl_seiten > 1 ) {
					...
					if( $news_page < $anzahl_seiten ) {
						...
						$nextpage = " | <a href=\"" . DLEUrl::BuildUrl('static.page', ['page' => $static_result['name'], 'news_page' => $pages]) . "\">" . $lang['news_next'] . "</a>";
					...
					if( $news_page > 1 ) {
						...
						$full_link = DLEUrl::BuildUrl('static.page', ['page' => $static_result['name'], 'news_page' => $news_page]);
						
						if ($pages == 1 ) {
							
							$prevpage = "<a href=\"" . DLEUrl::BuildUrl('static', ['page' => $static_result['name']]) . "\">" . $lang['news_prev'] . "</a> | ";

						} else {

							$prevpage = "<a href=\"" . DLEUrl::BuildUrl('static.page', ['page' => $static_result['name'], 'news_page' => $pages]) . "\">" . $lang['news_prev'] . "</a> | ";
					...
					$pages_navigation_link = DLEUrl::BuildUrl('static.page', ['page' => $static_result['name'] ]);
			...
			$print_link = DLEUrl::BuildUrl('static.print', ['page' => $static_result['name']]);
			...
			if ( preg_match( "#\\{text limit=['\"](.+?)['\"]\\}#i", $tpl->copy_template, $matches ) ) {
				...
				$stext = removeHiddenDleBlocks($stext);
			...
			if( $_GET['page'] == "dle-rules-page" ) if( $do != "register" ) {
				...
				$tpl->result['content'] = str_ireplace( '{ACCEPT-DECLINE}', "<form  method=\"post\" name=\"registration\" id=\"registration\" action=\"\"><button type=\"submit\" class=\"bbcodes\">{$lang['rules_accept']}</button> <button type=\"button\" class=\"bbcodes\" onclick=\"history.go(-1); return false;\">{$lang['rules_decline']}</button><input name=\"do\" type=\"hidden\" id=\"do\" value=\"register\"><input name=\"dle_rules_accept\" type=\"hidden\" id=\"dle_rules_accept\" value=\"yes\"></form>", $tpl->result['content'] );
		...
		if ($config['rss_informer'] && isset($informers) && is_array($informers) && count ($informers) ) {
			foreach ( $informers as $name => $value ) {
				$tpl->result['content'] = str_replace ( "{inform_" . $name . "}", $value, $tpl->result['content'] );
			}
		}
		
		if( $config['allow_banner'] && count( $banners ) ) {
		...
		if ($config['allow_links'] && $view_template != "print" && function_exists('replace_links') && isset($replace_links['static'])) {
