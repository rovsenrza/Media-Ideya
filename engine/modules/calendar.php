...
function cal($cal_month, $cal_year, $events) {
	global $f, $r, $config, $lang, $langdateshortweekdays;
	...
	$date_link['prev'] = '<a class="monthlink monthlink-prev" onclick="doCalendar(' . date("'m','Y'", $prev_of_month) . ',\'right\'); return false;" href="' . DLEUrl::BuildUrl('date.month', ['year' => date('Y', $prev_of_month), 'month' => date('m', $prev_of_month)]) . '" title="' . $lang['prev_moth'] . '">&laquo;</a>';
	$date_link['next'] = '<a class="monthlink monthlink-next" onclick="doCalendar(' . date("'m','Y'", $next_of_month) . ',\'left\'); return false;" href="' . DLEUrl::BuildUrl('date.month', ['year' => date('Y', $next_of_month), 'month' => date('m', $next_of_month)]) . '" title="' . $lang['next_moth'] . '">&raquo;</a>';
	
	if( ! $next ) $date_link['next'] = "<span class='monthlink monthlink-next'>&raquo;</span>";
	
	$buffer = '<div id="calendar-layer"><table id="calendar" class="calendar"><tr><th colspan="7" class="monthselect"><div class="monthselect-inner">' . $date_link['prev'] . '<span class="monthselect-title">' . langdate( 'F', $first_of_month, true ) . ' ' . $cal_year . '</span>' . $date_link['next'] . '</div></th></tr><tr>';
	...
	while ( $maxdays > $cal_day ) {
		...
		if( isset( $events[$cal_day] ) ) {
			...
			if( $weekday == '5' or $weekday == '6' ) {
				...
				$buffer .= '<td ' . (($cal_pos == $cur_date) ? ' class="day-active day-current" ' : ' class="day-active" ') . '><a class="day-active" href="'. DLEUrl::BuildUrl('date.day', ['year' => date('Y', $events[$cal_day]), 'month' => date('m', $events[$cal_day]), 'day' => date('d', $events[$cal_day])]) .'" title="' . $lang['cal_post'] . ' ' . $date['title'] . '">' . $cal_day . '</a></td>';
			
			} else {

				$buffer .= '<td ' . (($cal_pos == $cur_date) ? ' class="day-active-v day-current" ' : ' class="day-active-v" ') . '><a class="day-active-v" href="'. DLEUrl::BuildUrl('date.day', ['year' => date('Y', $events[$cal_day]), 'month' => date('m', $events[$cal_day]), 'day' => date('d', $events[$cal_day])]) . '" title="' . $lang['cal_post'] . ' ' . $date['title'] . '">' . $cal_day . '</a></td>';
...
if( $config['allow_archives'] ) {
	...
	if( ! $tpl->result['archive'] ) {
		...
		while ( $row = $db->get_row() ) {
			...
			$news_archive[] = '<a class="archives" href="' . DLEUrl::BuildUrl('date.month', ['year' => $arch_url[1], 'month' => $arch_url[0]]). '"><b>' . $arch_title['ru'] . ' (' . $row['cnt'] . ')</b></a>';
