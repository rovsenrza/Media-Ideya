...
$canonical = DLEUrl::BuildUrl('statistics', []);
...
if( ! $tpl->result['content'] ) {
	...
	while ( $row = $db->get_row() ) {
		...
		$user_name = "onclick=\"ShowProfile('" . urlencode( $row['name'] ) . "', '" . $u_url . "', '" . $user_group[$member_id['user_group']]['admin_editusers'] . "'); return false;\"";
		$user_name = "<a {$user_name} class=\"pm_list\" href=\"" . $u_url . "\">" . $row['name'] . "</a>";
			
		$user_pm = "<a href=\"{$_SERVER['SCRIPT_NAME']}?do=pm&amp;doaction=newpm&amp;username=" . urlencode($row['name']) . "\">{$lang['top_pm']}</a>";
