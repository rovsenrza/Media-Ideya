        ...
        <tr>
                <td class="search" colspan="2">
                    <div style="margin-top:6px">
                        <button type="button" class="bbcodes" name="dosearch" id="dosearch" onclick="javascript:list_submit(-1); return false;">{$lang['s_fstart']}</button>
                        <button type="button" class="bbcodes" name="doclear" id="doclear" onclick="javascript:clearform('fullsearch'); return false;">{$lang['s_fstop']}</button>
                    </div>
...
HTML;
		...
		if ( $smartphone_detected ) {
			...
			$link_full_search = "<button type=\"button\" class=\"bbcodes\" name=\"dofullsearch\" id=\"dofullsearch\" onclick=\"javascript:full_submit(1); return false;\">{$lang['s_ffullstart']}</button>";
	...
	if( $subaction == "search" ) {
		...
		if( (empty( $story ) or ($story_maxlen < $config['search_length_min'])) and (empty( $searchuser ) or (strlen( $searchuser ) < $config['search_length_min'])) ) {
			...
			if( in_array( $titleonly, array (0, 3) ) ) {
				...
				$posts_fields = "SELECT p.id, p.autor, p.date, p.short_story, (p.full_story != '') as full_story, p.xfields, p.title, p.descr, p.keywords, p.category, p.alt_name, p.comm_num, p.allow_comm, p.allow_main, p.approve, p.fixed, p.symbol, p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.disable_index, e.editdate, e.editor, e.reason{$user_select}{$full_s_addfield}";
				...
				if( $titleonly == 2 ) {
					...
					while ( $row = $db->get_row( $sql_result ) ) {
						...
						if( $anzahl_seiten > 1 ) {
							...
							$pages_navigation_link = DLEUrl::BuildUrl('static.page', ['page' => $row['static_name'] ]);
						...
						if( $row['allow_template'] ) {
							...
							$static_descr = "<a href=\"" . DLEUrl::BuildUrl('static', ['page' => $row['static_name']]) . "\" >" . $title . "</a>";
							...
							if (dle_strlen( $row['story'] ) > 2000) {
								...
								$row['story'] = removeHiddenDleBlocks($row['story']);
								...
								$row['story'] .= "( <a href=\"" . DLEUrl::BuildUrl('static', ['page' => $row['static_name']]) . "\" >" . $lang['search_s_go'] . "</a> )";
							...
							$print_link = DLEUrl::BuildUrl('static.print', ['page' => $row['static_name']]);
