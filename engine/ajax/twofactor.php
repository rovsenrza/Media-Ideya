...
include_once(DLEPlugins::Check(ENGINE_DIR . '/classes/twofactor/twofactor.php'));
...
if( isset($_GET['mode']) AND $_GET['mode'] == 'createsecret') {
	...
	try {
		...
		$url = $tfa->getQRCodeImageAsDataUri($member_id['email'], $secret);
	} catch (\Throwable $e) {
	...
	if ( !isset($_SESSION['twofactor_secret']) OR !$_SESSION['twofactor_secret'] ) {
	...
	if (!isset($_POST['pin']) OR !$_POST['pin']) {
		echo "{\"error\":true, \"errorinfo\":\"{$lang['twofactor_err_2']}\"}";
		die();
	}
	...
	if ($member_id['user_id'] AND $member_id['password'] AND $_SESSION['twofactor_auth'] AND md5($member_id['password']) == $_SESSION['twofactor_auth']) {
		...
		if (isset($_SESSION['no_save_cookie']) AND $_SESSION['no_save_cookie']) {
