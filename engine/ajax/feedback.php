	...
	if ( !isset($row['id']) OR !$row['id'] OR !$row['email'] ) {
		...
		if ($sfs->is_spammer( $args )) {
			...
			if ( !isset($row['id']) OR !$row['id'] ) {
				$db->query( "INSERT INTO " . PREFIX . "_spam_log (ip, is_spammer, email, date) VALUES ('{$_IP}','1', '". $db->safesql($email)."', '{$_TIME}')" );
			} else {
				$db->query( "UPDATE " . PREFIX . "_spam_log SET is_spammer='1', email='" . $db->safesql($email) . "' WHERE id='{$row['id']}'" );
			}
			...
			if ( !isset($row['id']) OR !$row['id'] ) {
				$db->query( "INSERT INTO " . PREFIX . "_spam_log (ip, is_spammer, email, date) VALUES ('{$_IP}','0', '" . $db->safesql($email) . "', '{$_TIME}')" );
			} else {
				$db->query( "UPDATE " . PREFIX . "_spam_log SET email='" . $db->safesql($email) . "' WHERE id='{$row['id']}'" );
			}
		}
...
if( !count($stop) AND $user_group[$member_id['user_group']]['captcha_feedback'] ) {
	...
	if(!isset($_POST['sec_code']) OR !isset($_SESSION['sec_code_session']) OR $_POST['sec_code'] != $_SESSION['sec_code_session'] OR !$_SESSION['sec_code_session'] ) {
...
if( !count($stop) AND $user_group[$member_id['user_group']]['feedback_question'] ) {
...
if( !count($stop) AND $user_group[$member_id['user_group']]['allow_mail_files'] ) {
				...
				foreach ($file['name'] as $count => $i) {
					...
					if( $file['error'][$count] === UPLOAD_ERR_OK ) {
						...
						if( in_array($type, $allowed_files ) ) {
							...
							$stop[] = $lang['mail_file_err_2'];
							break;
						}

					} else $stop[] = $lang['mail_file_err_1'];
