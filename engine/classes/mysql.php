...
class db {
	...
	function connect($db_user, $db_pass, $db_name, $db_location = 'localhost', $show_error=1) {
		...
		if (stripos($this->mysql_version, 'mariadb') !== false && strpos($this->mysql_version, '5.5.5-') === 0) {
			$this->mysql_version = substr($this->mysql_version, 6);
		}

		$version_length = strspn($this->mysql_version, '0123456789.');

		if ($version_length) {
			$this->mysql_version = substr($this->mysql_version, 0, $version_length);
		}
	...
	function query($query, $show_error=true, $log_query=true) {
		...
		if(!($this->query_id = mysqli_query($this->db_id, $query) )) {
			...
			$query_time = $this->get_real_time() - $time_before;
			$this->MySQL_time_taken += $query_time;
	...
	function multi_query($query, $show_error=true, $log_query=true) {
		...
		if( mysqli_multi_query($this->db_id, $query) ) {
			...
			do {
				if ($result = mysqli_store_result($this->db_id)) {
