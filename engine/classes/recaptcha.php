...
class ReCaptcha
{
    ...
    {   global $config;
        ...
        if ($config['allow_recaptcha'] == 5) {
            $this->_siteVerifyUrl = "https://smartcaptcha.yandexcloud.net/validate";
            $this->_yandex = true;
        }
    ...
    public function verifyResponse($remoteIp, $response)
    {
        ...
        if($this->_yandex) { 
            if (isset($answers['status']) AND $answers['status'] == 'ok') {
                $recaptchaResponse->success = true;
            } else {
                $recaptchaResponse->success = false;
                $recaptchaResponse->errorCodes = isset($answers['message']) ? $answers['message'] : 'invalid-response';
            }
        } elseif (isset($answers['success']) AND $answers['success'] == true) {
