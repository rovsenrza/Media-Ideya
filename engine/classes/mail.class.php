...
class dle_mail {
	...
	function __construct($config, $is_html = false) {
		...
		try {
			...
			if( isset($lang['language_code']) AND $lang['language_code'] ) {
				PHPMailer::setLanguage( (string)$lang['language_code'], ROOT_DIR . '/engine/classes/mail/language/');
			}
			
			$config['admin_mail'] = stripslashes($config['admin_mail']);
...
//			$this->mail->SMTPDebug = SMTP::DEBUG_SERVER;
	...
	function send($to, $subject, $message) {
		...
		try {
			...
			if ($this->from) {
				$this->mail->addReplyTo(stripslashes( (string)$this->from ), stripslashes( (string)$this->from) );
			}

			$this->mail->addAddress( stripslashes((string)$to) );
			...
			if (count($this->bcc)) {
				...
				foreach ($this->bcc as $bcc) {
					$this->mail->addBCC(stripslashes((string)$bcc));
				}
			}
