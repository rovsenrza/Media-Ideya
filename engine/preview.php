...
if (!isset($_POST) OR !is_array($_POST) OR !count($_POST) ) {
	header( "HTTP/1.1 403 Forbidden" );
	header ( 'Location: /' );
	die("Hacking attempt!");
}
...
$catlist = filter_input(INPUT_POST, 'catlist', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY) ?: [];

$catlist = array_filter(
	array_map('intval', $catlist),
...
if( !count($catlist) ) {
	...
	if( isset($catlist[0]) AND isset($cat_info[$catlist[0]]['skin']) AND $cat_info[$catlist[0]]['skin'] ) {
		...
		if( @is_dir ( ROOT_DIR . '/templates/' . $cat_info[$catlist[0]]['skin'] ) ) {
			...
			$config['skin'] = $cat_info[$catlist[0]]['skin'];
		}
	}
