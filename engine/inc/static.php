...
if( $action == "list" ) {
	...
	while ( $row = $db->get_array() ) {
		...
		$vlink =  DLEUrl::BuildUrl('static', ['page' => $row['name']]);
  ...
  <div class="modal-body">
	...
	<div class="row">	  
	  <div class="col-sm-6">
		  ...
		  <div class="col-sm-12">
			{$lang['edit_fdate']} <input data-rel="calendardate" type="text" dir="auto" name="fromnewsdate" id="fromnewsdate" class="form-control" style="width:11.11em;" value="{$fromnewsdate}" autocomplete="off">
			{$lang['edit_tdate']} <input data-rel="calendardate" type="text" dir="auto" name="tonewsdate" id="tonewsdate" class="form-control" style="width:11.11em;" value="{$tonewsdate}" autocomplete="off">
		  </div>
		 </div>
...
<div class="panel panel-default">
  <div class="panel-heading">
    ...
    <table class="table table-striped table-xs table-hover">
      <thead>
      <tr>
        <th class="hidden-xs" style="width: 27.78em">{$lang['static_title']}</th>
        <th>{$lang['static_descr']}</th>
        <th class="hidden-xs" style="width: 4.17em;text-align:center;"><i class="fa fa-eye tip" data-original-title="{$lang['st_views']}"></i></th>
        <th style="width: 2.78em"><input class="icheck" type="checkbox" name="master_box" title="{$lang['edit_selall']}" onclick="javascript:ckeck_uncheck_all()"></th>
      </tr>
      </thead>
	  <tbody>
		{$entries}
...
HTML;
    ...
    function preview(){
		...
		if(document.static.template.value == '' || document.static.description.value == '' || document.static.name.value == ''){ DLEPush.error('{$lang['static_err_1']}'); }
			...
			\$('body').prepend(\"<div id='modal-overlay' style='opacity:0;display:none;'></div>\");
			\$('#modal-overlay').show();
			...
			document.static.mod.value='preview';document.static.target='preview_iframe';document.static.submit();
	...
	function auto_keywords ( key, ai ) {
		...
		var content = document.getElementById('template').value;
		...
		$.post(\"index.php?controller=ajax&mod=keywords\", { short_story: content, key: key, ai: ai, user_hash: '{$dle_login_hash}' }, function(data){
...
<div class="panel panel-default">
  ...
  <div class="panel-body">
		...
		<div class="form-group">
		  ...
		  <div class="col-md-10">
			<input data-rel="calendar" type="text" dir="auto" name="newdate" class="form-control position-left" style="width:13.19em;" value="" autocomplete="off"><label class="checkbox-inline"><input class="icheck" type="checkbox" name="allow_now" id="allow_now" value="yes" checked>{$lang['edit_jdate']}</label>
		  </div>
		 </div>
...
HTML;
	...
	echo <<<HTML
		...
		<div class="form-group">
		  ...
		  <div class="col-md-10">
			<select name="grouplevel[]" class="cat_select" data-placeholder="{$lang['group_select_1']}" style="width:17.36em;" multiple><option value="all" selected>{$lang['edit_all']}</option>{$groups}</select>
		  </div>
		 </div>
		<div class="form-group">
...
HTML;
	...
	$full_link = DLEUrl::BuildUrl('static', ['page' => $name]);
...
HTML;
	...
	if( $row['grouplevel'] == "all" ) $check_all = "selected"; else $check_all = "";
...
HTML;
	...
	echo <<<HTML
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['edit_edate']}</label>
		  <div class="col-md-10">
			<input data-rel="calendar" type="text" dir="auto" name="newdate" value="{$itemdate}" class="form-control position-left" style="width:13.19em;" autocomplete="off"><label class="checkbox-inline"><input class="icheck" type="checkbox" name="allow_now" id="allow_now" value="yes">{$lang['edit_jdate']}</label>
		  </div>
		 </div>
		<div class="form-group editor-group">
		  <label class="control-label col-md-2">{$lang['static_templ']}</label>
...
HTML;
	...
	echo <<<HTML
		...
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['meta_keys']}</label>
		  <div class="col-md-10">
			<textarea dir="auto" class="tags" name="keywords" id='keywords'>{$row['metakeys']}</textarea>
			<div class="mt-10">
				<input type="button" onclick="auto_keywords(1); return false;" class="btn bg-primary-600 btn-sm btn-raised position-left" value="{$lang['btn_descr']}">
				<input type="button" onclick="auto_keywords(2); return false;" class="btn bg-primary-600 btn-sm btn-raised" value="{$lang['btn_keyword']}">
			</div>
...
HTML;
	...
	if( isset($_GET['page']) AND $_GET['page'] == "rules" ) {
		...
		$full_link = DLEUrl::BuildUrl('rules', []);
