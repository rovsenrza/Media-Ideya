...
<div class="panel panel-default">
  ...
  <div class="panel-body">
	  ...
	  <div class="col-md-3">
		<select style="width:100%;height: 12.65em;" name="ta[]" multiple="multiple">{$tabellen}</select>
	  ...
	  <div class="col-md-9">
		<table width="100%">
          <tr>
            <td style="width:5em;"><i class="fa fa-retweet" style="font-size:400%"></i></td>
            <td width="5%" nowrap="nowrap"><div align="center">
                <input type="radio" name="whattodo" checked="checked" value="optimize" class="icheck">
              </div></td>
          ...
          <tr>
            ...
            <td width="5%" nowrap="nowrap"><div align="center">
                <input type="radio" name="whattodo" value="repair" class="icheck">
              </div></td>
...
<script>
    function save(){
		...
		var wh = 360 * getBaseSize();

		if(ww > ( $(window).width() * 0.95 ) )  { ww = $(window).width() * 0.95;  }

		$('body').append('<div id="modal-overlay" style="opacity:0;display:none;"></div>');
		$('#modal-overlay').show();
		$('#modal-overlay').css('opacity','1');

		$("#dlepopup").remove();
		$("body").append("<div id='dlepopup' class='postpreview-body' title='{$lang['db_back']}' style='display:none'></div>");
		...
		$('#dlepopup').dialog({
			...
			open: function(event, ui) { 
				$("#dlepopup").html("<iframe width='100%' height='100%' src='?mod=dumper&user_hash={$dle_login_hash}&action=backup&comp_method=" + $("#comp_method").val() + "&rndval=" + rndval + "' frameborder='0' marginwidth='0' marginheight='0' scrolling='no'></iframe>");
			},
...
<script>
    function dbload(){
		...
		$("#dlepopup").remove();
		$("body").append("<div id='dlepopup' class='postpreview-body' title='{$lang['db_load']}' style='display:none'></div>");
		...
		$('#dlepopup').dialog({
			...
			open: function(event, ui) { 
				$("#dlepopup").html("<iframe width='100%' height='100%' src='?mod=dumper&user_hash={$dle_login_hash}&action=restore&file=" + $("#file").val() + "&rndval=" + rndval + "' frameborder='0' marginwidth='0' marginheight='0' scrolling='no'></iframe>");
			},
			beforeClose: function(event, ui) { 
				$("#dlepopup").html("");
			},
