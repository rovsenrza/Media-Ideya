...
define('DINITVERSION', '3284' );
define('VERSIONID',    '20.0' );
define('BUILDID',      '109' );
...
if( function_exists('getSafeScriptName') ) {
	$_SERVER['SCRIPT_NAME'] = $_SERVER['PHP_SELF'] = getSafeScriptName();
}

if( !$config['http_home_url'] ) {

	$config['http_home_url'] = (isSSL() ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . ((($temp = strrpos($_SERVER['SCRIPT_NAME'], '/')) !== false && $temp > 0) ? substr($_SERVER['SCRIPT_NAME'], 0, $temp) : '') . '/';
	unset($temp);
