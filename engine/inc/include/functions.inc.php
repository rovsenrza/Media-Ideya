...
function clear_cache($cache_areas = false) {
	...
	while ( $file = readdir( $fdir ) ) {
		if( !is_dir(ENGINE_DIR . '/cache/' . $file) ) {
...
function clear_all_caches() {
	...
	while ( $file = readdir( $fdir ) ) {
		if( !is_dir(ENGINE_DIR . '/cache/system/' . $file ) AND $file != 'cron.json' ) {
...
function create_metatags($story, $ajax = false) {
	...
	if( $_REQUEST['keywords'] ) {
		...
		$story = clear_content(stripslashes($story), 0, false);
...
function get_folder_list( $folder = 'language' ) {
	...
	if($folder == 'language') {
		if (isset($list['Russian'])) {
			$list = ['Russian' => $list['Russian']] + $list;
		}

		foreach(['Indonesian', 'Vietnamese', 'Thai', 'Hindi', 'Chinese', 'Arabic', 'Hebrew', 'Persian'] as $lang_name) {
			if (isset($list[$lang_name])) {
				$temp_lang = $list[$lang_name];
				unset($list[$lang_name]);
				$list[$lang_name] = $temp_lang;
			}
		}
	}
...
function build_js($js) {
	...
	$default_array = array(
		"public/js/jquery4.js",
		"public/js/jqueryui.js",
		'public/adminpanel/javascripts/application.js',
	);
...
function build_css($css) {
	...
	$default_array = array (
		"public/fonts/fontawesome/styles.min.css",
		"public/adminpanel/stylesheets/application{$rtl_prefix}.css"
	);
...
function clearfilepath( $file, $ext=array() ) {
	...
	elseif( $filename ) return $filename;
