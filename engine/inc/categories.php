...
$home_url = substr($_SERVER['SCRIPT_NAME'], 0, (int)strrpos($_SERVER['SCRIPT_NAME'], '/') + 1) ?: '/';
...
if( $action == "add" ) {
	...
	if( $row['count'] ) {
		...
		if( isset($_POST['subaction']) AND $_POST['subaction'] == 2 ) {
			...
			msg( "warning", $lang['all_info'], "<form action=\"\" method=\"post\">{$lang['comm_move']} <select name=\"new_category[]\" class=\"categoryselect\" data-placeholder=\"{$lang['addnews_cat_sel']}\" style=\"width:24.31em;\" multiple>" . CategoryNewsSelection( 0, 0 ) . "</select> <input class=\"btn bg-primary btn-sm btn-raised\" type=\"submit\" value=\"{$lang['b_start']}\"><script>
$(function(){
...
<div class="panel panel-default">
  ...
  <div class="panel-body">
		...
		<div class="form-group">
		  ...
		  <div class="col-md-10 col-sm-9">
			<textarea dir="auto" name="fulldescr" class="classic" style="width:100%;max-width:38.19em;" rows="5">{$row['fulldescr']}</textarea>
		  </div>
		 </div>
		<div class="form-group">
		...
		<div class="form-group">
		  ...
		  <div class="col-md-10 col-sm-9">
			<textarea dir="auto" name="keywords" class="classic" style="width:100%;max-width:38.19em;" rows="3">{$row['keywords']}</textarea>
		  </div>
		 </div>
		<div class="form-group">
		  <label class="control-label col-md-2 col-sm-3">{$lang['cat_parent']}</label>
		  <div class="col-md-10 col-sm-9">
			<select class="uniform" name="parentid" data-width="350" style="width:100%;max-width:24.31em;" data-live-search="true" data-none-results-text="{$lang['addnews_cat_fault']}">{$categorylist}</select>
		  </div>
		 </div>
		<div class="form-group">
		  <label class="control-label col-md-2 col-sm-3">{$lang['cat_skin']}</label>
		  <div class="col-md-10 col-sm-9">
			<div style="width:15.28em;display:inline-block;">{$skinlist}</div><i class="help-button visible-lg-inline-block text-primary-600 fa fa-question-circle position-right" data-rel="popover" data-trigger="hover" data-placement="auto right" data-content="{$lang['hint_cattempl']}"></i>
		  </div>
		 </div>
		<div class="form-group">
		...
		<div class="form-group">
		  ...
		  <div class="col-md-10 col-sm-9">		
			<div class="row">
				<div class="col-sm-6" style="max-width:20.83em;"><div class="checkbox"><label><input class="icheck" type="checkbox" name="disable_main" value="1" {$ifch2}>{$lang['cat_d_main']}</label></div></div>
			...
			<div class="row">
				<div class="col-sm-6" style="max-width:20.83em;"><div class="checkbox"><label><input class="icheck" type="checkbox" name="disable_rating" value="1" {$ifch4}>{$lang['cat_d_rating']}</label></div></div>
			...
			<div class="row">
				<div class="col-sm-6" style="max-width:20.83em;"><div class="checkbox"><label><input class="icheck" type="checkbox" name="enable_dzen" value="1" {$ifch5}>{$lang['cat_dzen']}</label></div></div>
...
jQuery(function($){
	...
	var uploader = new plupload.Uploader({
		...
		url: "index.php?controller=ajax&mod=upload",
		multipart_params: {"subaction" : "upload", "news_id" : "0", "area" : "adminupload", "userdir" : "icons", "upload_driver" : "0", "user_hash" : "{$dle_login_hash}"},
		...
		if( count( $cat_info ) ) {
			...
			if( count( $root_category ) ) {
				...
				foreach ( $root_category as $id ) {
					...
					$link = "<a href=\"" . DLEUrl::BuildUrl('category', ['category' => get_url($id)]) . "\" target=\"_blank\">" . stripslashes( $cat_info[$id]['name'] ) . "</a>";
