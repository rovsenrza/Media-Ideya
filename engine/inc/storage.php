...
HTML;
	...
	showRow($lang['opt_sys_imfsfp'], $lang['opt_sys_imfsfpd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:6.94em; text-align: center;\"  name='storage[connect_port]' value=\"{$row['connect_port']}\" >", "ftp-server");
...
HTML;
		...
		$entries .= "<tr class=\"dd-item dd-item-table drag-bg\" data-id=\"{$row['id']}\">
...
<div class="dd table-responsive">
    <table class="table table-xs table-hover">
      ...
      <tr>
		...
		<th style="width: 22.22em">{$lang['storage_type']}</th>
	  ...
	  <tbody class="dd-list dd-table" id="nestable">
		{$entries}
	  </tbody>
	</table>
</div>
...
$(function(){
    ...
    if( document.getElementById('nestable') ) {
		var storage_sort = new Sortable(document.getElementById('nestable'), {
			group: "rubric",
			animation: 150,
			ghostClass: 'drop-bg',
			handle: '.dd-handles',
			...
			onSort: function (evt) {
				var url = "action=storagesort&user_hash={$dle_login_hash}&list="+window.JSON.stringify(storage_sort.toArray());
