...
if( $action == "addnews" ) {
	...
	$id='';
	$duplicateprotection = UniqIDReal(32);
...
HTML;	
	...
	function find_related_ids (){
		...
		$.post(\"index.php?controller=ajax&mod=adminfunction\", { action: 'relatedids', title: title, short_txt: short_txt, full_txt: full_txt, category: catselected, user_hash: '{$dle_login_hash}' }, function(data){
	...
	function find_relates ( )
	{
		...
		$.post('index.php?controller=ajax&mod=find_relates', { title: title, user_hash: '{$dle_login_hash}' }, function(data){
	...
	function xfimagedelete( xfname, xfvalue )
	{
		...
		DLEconfirmDelete( '{$lang['image_delete']}', '{$lang['p_info']}', function () {
			...
			$.post('index.php?controller=ajax&mod=upload', { subaction: 'deluploads', user_hash: '{$dle_login_hash}', news_id: '', author: '', 'images[]' : xfvalue }, function(data){
	...
	function xffiledelete( xfname, xfvalue )
	{
		DLEconfirmDelete( '{$lang['file_delete']}', '{$lang['p_info']}', function () {
			...
			$.post('index.php?controller=ajax&mod=upload', { subaction: 'deluploads', user_hash: '{$dle_login_hash}', 'files[]' : xfvalue }, function(data){
	...
	if( $member_id['user_group'] < 3 ) {
		...
		$author_info = "<span class=\"position-left visible-lg-inline-block visible-md-inline-block visible-sm-inline-block visible-xs\">{$lang['edit_eau']}</span><input type=\"text\" dir=\"auto\" name=\"new_author\" id=\"new_author\" class=\"form-control\" style=\"width: 11.11em;\" value=\"{$member_id['name']}\">";
						...
						<div class="panel-body">
							...
							<div class="form-group">
							 ...
							 <div class="form-group">
							  ...
							  <div class="col-sm-10">
								<select data-placeholder="{$lang['addnews_cat_sel']}" title="{$lang['addnews_cat_sel']}" name="category[]" id="category" onchange="onCategoryChange(this)" $category_multiple style="width:100%;max-width:24.3em;">{$categories_list}</select><input type="hidden" name="category_custom_sort" id="category_custom_sort" value="">
							  </div>
							</div>
HTML;
						...
						<div class="panel-body">
						 ...
						 <div class="panel-body">	 
							...
							<div class="form-group">
							  ...
							  <div class="col-md-10 col-sm-9">
								<textarea dir="auto" class="tags" name="keywords" id="keywords"></textarea>
									<div class="mt-10">
...
elseif( $action == "doaddnews" ) {
	...
	$expires_action = (int)($_POST['expires_action'] ?? 0);
