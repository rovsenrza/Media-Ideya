...
HTML;
	  ...
	  <div class="col-sm-6 media-list media-list-linked">
		<a class="media-link" href="{$option['url']}">
			<div class="media-left"><img src="public/adminpanel/images/{$option['image']}" class="img-lg section_icon"></div>
...
HTML;
	...
	while ( $row = $db->get_array() ) {
		...
		if ( !$row['icon'] OR !@file_exists( $row['icon'] )) $row['icon'] = "public/adminpanel/images/default_icon.png";
...
HTML;
	...
	$config['offline_reason'] = str_replace('<br>', "\n", $config['offline_reason']);
		...
		foreach ( $options as $value => $description ) {
			...
			if(is_array( $description )) {
				if( isset( $description['icon'] ) AND $description['icon'] ) {
					$output .= " data-content=\"<span class='select-icon'><img src='public/flags/{$description['icon']}'></span><span class='select-descr'>{$description['name']}</span>\" ";
				}
...
<!--
	...
	function ShowOrHideAIComments(obj) {
		if( $(obj).is(':checked') ) {
			$(".ai-params-comments").show();
		} else {
			$(".ai-params-comments").hide();
		}
	}
	function ShowOrHideAICommentsStart(value) {
		if( value == '1' ) {
			$(".ai-params-comments").show();
		} else {
			$(".ai-params-comments").hide();
	...
	function change_domain() {
		...
		$("body").append("<div id='dlepopup' title='{$lang['change_domain']}' style='display:none'>{$lang['old_domain']}<br><input type='text' dir='auto' name='dle-promt-oldurl' id='dle-promt-oldurl' class='form-control' style='width:100%;' value='{$config['http_home_url']}'/><br><br>{$lang['new_domain']}<br><input type='text' dir='auto' name='dle-promt-newurl' id='dle-promt-newurl' class='form-control' style='width:100%;' value=''><br><span class='text-muted text-size-small'>{$lang['change_domain_h']}</span></div>");
...
HTML;
	...
	showRow( $lang['opt_sys_key'], $lang['opt_sys_keyd'], "<textarea dir=\"auto\" class=\"classic\" style=\"width:100%;\" rows=\"3\" name=\"save_con[keywords]\">{$config['keywords']}</textarea>" );
	...
	showRow($lang['opt_sys_ht2'], $lang['opt_sys_ht2d'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" name=\"save_con[short_meta_title]\" value=\"{$config['short_meta_title']}\">");
	showRow($lang['opt_sys_ht3'], $lang['opt_sys_ht3d'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[sep_meta_title]\" value=\"{$config['sep_meta_title']}\">");
	...
	showRow( $lang['opt_sys_jqv'], $lang['opt_sys_jqvd'], makeDropDown( array ("0" => "jQuery 2.xx", "3" => "jQuery 3.xx", "4" => "jQuery 4.xx"), "save_con[jquery_version]", "{$config['jquery_version']}" ) );
	...
	showRow( $lang['opt_sys_reason'], $lang['opt_sys_reasond'], "<textarea dir=\"auto\" class=\"classic\" style=\"width:100%;\" rows=\"7\" name=\"save_con[offline_reason]\">{$config['offline_reason']}</textarea>" );
...
HTML;
	...
	showRow($lang['opt_sys_coua5'], $lang['opt_sys_couad5'], "<textarea dir=\"auto\" class=\"classic\" style=\"width:100%;\" rows=\"3\" name=\"save_con[country_decline_reason]\">{$config['country_decline_reason']}</textarea>");
	...
	showRow( $lang['opt_sys_iprest'], $lang['opt_sys_iprestd'], "<textarea dir=\"auto\" class=\"classic\" style=\"width:100%;\" rows=\"3\" name=\"save_con[admin_allowed_ip]\">{$config['admin_allowed_ip']}</textarea>" );
	showRow( $lang['opt_sys_llog'], $lang['opt_sys_llogd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[login_log]\" value=\"{$config['login_log']}\">" );
	showRow( $lang['opt_sys_tban'], $lang['opt_sys_tband'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[login_ban_timeout]\" value=\"{$config['login_ban_timeout']}\">" );
	showRow( $lang['opt_sys_tsess'], $lang['opt_sys_tsessd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[session_timeout]\" value=\"{$config['session_timeout']}\">" );
	...
	showRow( $lang['opt_sys_recapt'], $lang['opt_sys_recaptd'], makeDropDown( array ("0" => $lang['opt_sys_gd2'], "1" => 'reCAPTCHA v2', "2" => 'reCAPTCHA v3', "3" =>'hCaptcha', "4" => 'Cloudflare Turnstile', "5" => 'Yandex SmartCaptcha' ), "save_con[allow_recaptcha]", "{$config['allow_recaptcha']}" ) );
	...
	showRow( $lang['opt_sys_recaptsc'], $lang['opt_sys_recaptscd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[recaptcha_score]\" value=\"{$config['recaptcha_score']}\">" );

	showRow( $lang['opt_sys_mdl'], $lang['opt_sys_mdld'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[adminlog_maxdays]\" value=\"{$config['adminlog_maxdays']}\">" );
...
HTML;
	...
	showRow( $lang['opt_sys_newc'], $lang['opt_sys_newd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[news_number]\" value=\"{$config['news_number']}\">" );
	showRow( $lang['opt_sys_snumc'], $lang['opt_sys_snumd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[search_number]\" value=\"{$config['search_number']}\">" );
	showRow( $lang['opt_sys_findr'], $lang['opt_sys_findrd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[search_pages]\" value=\"{$config['search_pages']}\">" );
	showRow( $lang['opt_sys_minsearch'], $lang['opt_sys_minsearchd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[search_length_min]\" value=\"{$config['search_length_min']}\">" );
	showRow($lang['opt_sys_fsearchr'], $lang['opt_sys_fsearchrd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[fastsearch_result]\" value=\"{$config['fastsearch_result']}\">");
	showRow( $lang['opt_sys_related_num'], $lang['opt_sys_related_numd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[related_number]\" value=\"{$config['related_number']}\">" );
	showRow($lang['opt_sys_related_date'], $lang['opt_sys_related_dated'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[related_date]\" value=\"{$config['related_date']}\">");
	showRow($lang['opt_sys_rfc'], $lang['opt_sys_rfcd'], makeCheckBox("save_con[related_only_cats]", "{$config['related_only_cats']}"));
	showRow( $lang['opt_sys_top_num'], $lang['opt_sys_top_numd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[top_number]\" value=\"{$config['top_number']}\">" );
	showRow( $lang['opt_sys_cloud_num'], $lang['opt_sys_cloud_numd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[tags_number]\" value=\"{$config['tags_number']}\">" );
	showRow( $lang['opt_sys_max_mod'], $lang['opt_sys_max_modd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[max_moderation]\" value=\"{$config['max_moderation']}\">" );

	showRow( $lang['opt_sys_max_new'], $lang['opt_sys_max_newd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[post_new]\" value=\"{$config['post_new']}\">" );
	showRow( $lang['opt_sys_max_upd'], $lang['opt_sys_max_updd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[post_updated]\" value=\"{$config['post_updated']}\">" );


	showRow( $lang['group_n_restr'], $lang['group_n_restrd'], "<input  type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[news_restricted]\" value=\"{$config['news_restricted']}\">" );
	showRow( $lang['opt_sys_cls'], $lang['opt_sys_clsd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[category_separator]\" value=\"{$config['category_separator']}\">" );
	showRow( $lang['opt_sys_tls'], $lang['opt_sys_tlsd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[tags_separator]\" value=\"{$config['tags_separator']}\">" );
	showRow( $lang['opt_sys_spbs'], $lang['opt_sys_spbsd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[speedbar_separator]\" value=\"{$config['speedbar_separator']}\">" );
	...
	showRow( $lang['opt_sys_an'], "<a onclick=\"javascript:Help('date'); return false;\" href=\"#\">$lang[opt_sys_and]</a>", "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[timestamp_active]\" value=\"{$config['timestamp_active']}\">" );
	...
	showRow($lang['opt_sys_aim8'], $lang['opt_sys_aim8d'], makeCheckBox("save_con[ai_proxy]", "{$config['ai_proxy']}"), "ai-params");
	...
	showRow($lang['opt_sys_aim3'], $lang['opt_sys_aim3d'], "<input dir=\"auto\" type=\"text\" class=\"form-control\"  style=\"max-width:10.42em;\" name=\"save_con[ai_mode]\" value=\"{$config['ai_mode']}\" >", "ai-params" );
	showRow($lang['opt_sys_aim4'], $lang['opt_sys_aim4d'], "<input dir=\"auto\" type=\"text\" class=\"form-control\"  style=\"max-width:10.42em;\" name=\"save_con[ai_tokens]\" value=\"{$config['ai_tokens']}\" >", "ai-params");
	showRow($lang['opt_sys_aim7'], $lang['opt_sys_aim7d'], "<input dir=\"auto\" type=\"text\" class=\"form-control\"  style=\"max-width:10.42em;\" name=\"save_con[ai_completion_tokens]\" value=\"{$config['ai_completion_tokens']}\" >", "ai-params");

	showRow($lang['opt_sys_aim5'], $lang['opt_sys_aim5d'], "<input dir=\"auto\" type=\"text\" class=\"form-control\"  style=\"max-width:10.42em;\" name=\"save_con[ai_temperature]\" value=\"{$config['ai_temperature']}\" >", "ai-params");
...
HTML;
	...
	showRow( $lang['opt_sys_cpm'], $lang['opt_sys_cpmd'], "<input  type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[comm_nummers]' value=\"{$config['comm_nummers']}\">" );
	showRow( $lang['opt_sys_cpml'], $lang['opt_sys_cpmld'], "<input  type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[last_comm_nummers]' value=\"{$config['last_comm_nummers']}\">" );
	showRow( $lang['opt_sys_trcl'], $lang['opt_sys_trcld'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[tree_comments_level]\" value=\"{$config['tree_comments_level']}\">" );
	...
	showRow( $lang['group_c_restr'], $lang['group_c_restrd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name=\"save_con[comments_restricted]\" value=\"{$config['comments_restricted']}\">" );
	...
	showRow( $lang['opt_sys_mcommd'], $lang['opt_sys_mcommdd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[max_comments_days]' value=\"{$config['max_comments_days']}\">" );
	showRow( $lang['opt_sys_minc'], $lang['opt_sys_mincd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[comments_minlen]' value=\"{$config['comments_minlen']}\">" );
	showRow( $lang['opt_sys_maxc'], $lang['opt_sys_maxcd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[comments_maxlen]' value=\"{$config['comments_maxlen']}\">" );
	...
	showRow( $lang['opt_sys_aw'], $lang['opt_sys_awd'], "<input  type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[auto_wrap]' value=\"{$config['auto_wrap']}\">" );
	showRow( $lang['opt_sys_ct'], "<a onclick=\"javascript:Help('date'); return false;\" href=\"#\">$lang[opt_sys_and]</a>", "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name='save_con[timestamp_comment]' value=\"{$config['timestamp_comment']}\">" );
	...
	showRow($lang['opt_sys_wdpm'], $lang['opt_sys_wdpmd'], makeDropDown(array("0" => $lang['editor_none'], "1" => $lang['editor_wisywig']), "save_con[allow_pm_wysiwyg]", "{$config['allow_pm_wysiwyg']}"));

	if(!$config['allow_cmod']) $lang['opt_sys_aim10d'] .= '<br><br><span style="color:red">'. $lang['opt_sys_aim12d']. '</span>';
	showRow($lang['opt_sys_aim9'], $lang['opt_sys_aim9d'], makeCheckBox("save_con[enable_ai_comments]", "{$config['enable_ai_comments']}", "onchange=\"ShowOrHideAIComments(this)\""));
	showRow($lang['opt_sys_aim10'], $lang['opt_sys_aim10d'], makeDropDown(array("1" => $lang['action_ai_1'], "2" => $lang['action_ai_2'], "3" => $lang['action_ai_3'], "4" => $lang['action_ai_4']), "save_con[comment_ai_action]", "{$config['comment_ai_action']}"), "ai-params-comments");
	if (!$config['allow_cmod']) $lang['opt_sys_aim13d'] .= '<br><br><span style="color:red">' . $lang['opt_sys_aim12d'] . '</span>';
	showRow($lang['opt_sys_aim13'], $lang['opt_sys_aim13d'], makeDropDown(array("0" => $lang['action_ai_0'], "1" => $lang['action_ai_1'], "2" => $lang['action_ai_2']), "save_con[comment_ai_down]", "{$config['comment_ai_down']}"), "ai-params-comments");
	$ai_comments_groups = get_groups(explode(',', $config['ai_comments_groups']));
	showRow($lang['opt_sys_aim11'], $lang['opt_sys_aim11d'], "<select name=\"ai_comments_groups[]\" class=\"cat_select\" data-placeholder=\"{$lang['group_select_1']}\" style=\"width:250px;\" multiple>{$ai_comments_groups}</select>", "ai-params-comments");

	showRow($lang['opt_sys_aim2'], $lang['opt_sys_aim2d'], "<input dir=\"auto\" type=\"text\" class=\"form-control\"  name=\"save_con[ai_endpoint_comments]\" value=\"{$config['ai_endpoint_comments']}\" >", "ai-params-comments");
	showRow($lang['opt_sys_aim1'], $lang['opt_sys_aim1d'], "<input dir=\"auto\" type=\"text\" class=\"form-control\"  name=\"save_con[ai_key_comments]\" value=\"{$config['ai_key_comments']}\" >", "ai-params-comments");
	showRow($lang['opt_sys_aim3'], $lang['opt_sys_aim3d'], "<input dir=\"auto\" type=\"text\" class=\"form-control\"  style=\"max-width:10.42em;\" name=\"save_con[ai_model_comments]\" value=\"{$config['ai_model_comments']}\" >", "ai-params-comments");
...
HTML;
	...
	showRow( $lang['opt_sys_cc'], $lang['opt_sys_ccd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[clear_cache]\" value=\"{$config['clear_cache']}\">" );
	showRow( $lang['opt_sys_mcac'], $lang['opt_sys_mcacd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[max_cache_pages]\" value=\"{$config['max_cache_pages']}\">" );
	showRow( $lang['opt_sys_fc'], $lang['opt_sys_fcd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[fullcache_days]\" value=\"{$config['fullcache_days']}\">" );
	...
	showRow( $lang['opt_sys_rnctime'], $lang['opt_sys_rnctimed'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" name=\"save_con[read_count_time]\" value=\"{$config['read_count_time']}\">" );
...
HTML;
	...
	showRow( $lang['opt_sys_maxfilesh'], $lang['opt_sys_maxfileshd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[file_chunk_size]' value=\"{$config['file_chunk_size']}\">" );
...
HTML;
	...
	showRow( $lang['opt_sys_amail'], $lang['opt_sys_amaild'], "<input dir=\"auto\" type=\"text\" name='save_con[admin_mail]' value='{$config['admin_mail']}' class=\"form-control\" style=\"width:100%;max-width:17.36em;\">" );
	showRow( $lang['opt_sys_mt'], $lang['opt_sys_mtd'], "<input dir=\"auto\" type=\"text\" name='save_con[mail_title]' value=\"{$config['mail_title']}\" class=\"form-control\" style=\"width:100%;max-width:17.36em;\">" );
	showRow( $lang['opt_sys_mm'], $lang['opt_sys_mmd'], makeDropDown( array ("php" => "PHP Mail()", "smtp" => "SMTP" ), "save_con[mail_metod]", "{$config['mail_metod']}" ) );
	showRow( $lang['opt_sys_smtph'], $lang['opt_sys_smtphd'], "<input dir=\"auto\" type=\"text\" name='save_con[smtp_host]' value=\"{$config['smtp_host']}\" class=\"form-control\" style=\"width:100%;max-width:17.36em;\">" );
	showRow( $lang['opt_sys_smtpp'], $lang['opt_sys_smtppd'], "<input dir=\"auto\" type=\"text\" name='save_con[smtp_port]' class=\"form-control\" style=\"max-width:10.42em; text-align: center;\" value=\"{$config['smtp_port']}\">" );
	showRow( $lang['opt_sys_smtup'], $lang['opt_sys_smtpud'], "<input dir=\"auto\" type=\"text\" name='save_con[smtp_user]' value=\"{$config['smtp_user']}\" class=\"form-control\" style=\"width:100%;max-width:17.36em;\">" );
	showRow( $lang['opt_sys_smtupp'], $lang['opt_sys_smtpupd'], "<input dir=\"auto\" type=\"text\" name='save_con[smtp_pass]' value=\"{$config['smtp_pass']}\" class=\"form-control\" style=\"width:100%;max-width:17.36em;\">" );
	...
	showRow( $lang['opt_sys_smtpm'], $lang['opt_sys_smtpmd'], "<input dir=\"auto\" type=\"text\" name='save_con[smtp_mail]' value=\"{$config['smtp_mail']}\" class=\"form-control\" style=\"width:100%;max-width:17.36em;\">" );
...
HTML;
	...
	showRow( $lang['opt_sys_um'], $lang['opt_sys_umd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[max_users]' value=\"{$config['max_users']}\">" );
	showRow( $lang['opt_sys_ud'], $lang['opt_sys_udd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[max_users_day]' value=\"{$config['max_users_day']}\">" );
	showRow( $lang['opt_sys_cm'], $lang['opt_sys_cmd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[max_complaints]' value=\"{$config['max_complaints']}\">" );
	showRow( $lang['opt_sys_mpml'], $lang['opt_sys_mpmld'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[max_pm_list]' value=\"{$config['max_pm_list']}\">" );
...
HTML;
	...
	showRow( $lang['opt_sys_minside'], $lang['opt_sys_minsided'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[min_up_side]' value=\"{$config['min_up_side']}\" >");
	showRow( $lang['opt_sys_maxside'], $lang['opt_sys_maxsided'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[max_up_side]' value=\"{$config['max_up_side']}\" >" );
	...
	showRow( $lang['opt_sys_maxsize'], $lang['opt_sys_maxsized'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[max_up_size]' value=\"{$config['max_up_size']}\">" );
	showRow( $lang['opt_sys_dim'], $lang['opt_sys_dimd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[max_image_days]' value=\"{$config['max_image_days']}\">" );
	showRow( $lang['opt_sys_ia'], $lang['opt_sys_iad'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[max_image]' value=\"{$config['max_image']}\">" );
	showRow( $lang['opt_sys_mi'], $lang['opt_sys_mid'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[medium_image]' value=\"{$config['medium_image']}\">" );
	...
	showRow( $lang['opt_sys_ij'], $lang['opt_sys_ijd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[jpeg_quality]' value=\"{$config['jpeg_quality']}\">" );
	showRow( $lang['opt_sys_av'], $lang['opt_sys_avd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[avatar_size]' value=\"{$config['avatar_size']}\">" );
	showRow( $lang['opt_sys_imw'], $lang['opt_sys_imwd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[tag_img_width]' value=\"{$config['tag_img_width']}\">" );
	...
	showRow( $lang['opt_sys_im'], $lang['opt_sys_imd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[max_watermark]' value=\"{$config['max_watermark']}\">" );
	...
	showRow( $lang['opt_sys_imths'], $lang['opt_sys_imthsd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[watermark_font]' value=\"{$config['watermark_font']}\" >" );
	showRow( $lang['opt_sys_imthc'], $lang['opt_sys_imthcd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[watermark_color_dark]' value=\"{$config['watermark_color_dark']}\" >" );
	showRow( $lang['opt_sys_imtlc'], $lang['opt_sys_imtlcd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[watermark_color_light]' value=\"{$config['watermark_color_light']}\" >" );
	...
	showRow( $lang['opt_sys_imthop'], $lang['opt_sys_imthopd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[watermark_opacity]' value=\"{$config['watermark_opacity']}\">" );
...
HTML;
	...
	showRow( $lang['opt_sys_sm_n'], $lang['opt_sys_sm_nd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[mobile_news]' value=\"{$config['mobile_news']}\">" );
...
HTML;
	...
	showRow( $lang['opt_sys_nrss'], $lang['opt_sys_nrssd'], "<input dir=\"auto\" type=\"text\" class=\"form-control\" style=\"max-width:10.42em; text-align: center;\"  name='save_con[rss_number]' value=\"{$config['rss_number']}\">" );
	...
	echo <<<HTML
<div style="margin-bottom:2.08em;">
...
<script>
	$(function(){
		...
		ShowOrHideAICommentsStart('{$config['enable_ai_comments']}');
	});
</script>
HTML;
	...
	$save_con['allow_pm_wysiwyg'] = isset($save_con['allow_pm_wysiwyg']) ? intval($save_con['allow_pm_wysiwyg']) : 0;
	...
	$save_con['ai_proxy'] = isset($save_con['ai_proxy']) ? intval($save_con['ai_proxy']) : 0;
	$save_con['enable_ai_comments'] = isset($save_con['enable_ai_comments']) ? intval($save_con['enable_ai_comments']) : 0;
	...
	$save_con['http_home_url'] = rtrim(trim($save_con['http_home_url']), '/') . '/';
	if($save_con['only_ssl']) $save_con['http_home_url'] = str_replace("http://", "https://", $save_con['http_home_url']);

	foreach (['offline_reason', 'country_decline_reason'] as $key) {
		$save_con[$key] = str_replace(["\r", "\n"], ['', '<br>'], trim(strip_tags(stripslashes($save_con[$key]))));
	}

	$save_con['feedback_groups'] = implode(',', array_diff(array_map('intval', (is_array($_POST['feedback_groups'] ?? null) && $_POST['feedback_groups']) ? $_POST['feedback_groups'] : [1]), [5]));
	$save_con['ai_groups'] = implode(',', array_diff(array_map('intval', (is_array($_POST['ai_groups'] ?? null) && $_POST['ai_groups']) ? $_POST['ai_groups'] : [1]), [5]));
	$save_con['protected_groups'] = implode(',', array_diff(array_map('intval', (is_array($_POST['protected_groups'] ?? null) && $_POST['protected_groups']) ? $_POST['protected_groups'] : []), [5]));
	$save_con['ai_comments_groups'] = (is_array($_POST['ai_comments_groups'] ?? null) && $_POST['ai_comments_groups']) ? implode(',', array_map('intval', $_POST['ai_comments_groups'])) : '';
	
	$save_con['admin_allowed_ip'] = str_replace(["\r", "\n"], ['', '|'], trim($save_con['admin_allowed_ip']));
	$allowed_ip = array_filter(array_map('trim', explode('|', $save_con['admin_allowed_ip'])), fn($v) => filter_var(explode('/', str_replace('*', '0', $v))[0], FILTER_VALIDATE_IP));
	$save_con['admin_allowed_ip'] = implode('|', $allowed_ip);

	if($save_con['related_number'] != $config['related_number']) {
		$db->query( "UPDATE " . PREFIX . "_post_extras SET related_ids=''" );
	}
	
	if ($save_con['seo_type'] != $config['seo_type']) {
		DLEUrl::ChangeRules(intval($save_con['seo_type']));
	}

	if (!$save_con['allow_alt_url'] AND !$save_con['seo_type']) {
		$save_con['seo_type'] = 2;
	}

	if( $auto_detect_config ) $config['http_home_url'] = "";

	$save_con['sep_meta_title'] = html_entity_decode($save_con['sep_meta_title'], ENT_QUOTES | ENT_HTML5, 'UTF-8');

	foreach ($config as $key => $value) {
		if (!isset($save_con[$key])) {
			$config[$key] = html_entity_decode($config[$key], ENT_QUOTES | ENT_XML1, 'UTF-8');
		}
	}
	...
	$keys = array_map(function ($key) {
		return totranslit($key, false, false);
	}, array_keys($save_con));
