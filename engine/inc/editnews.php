...
if( $action == "list" ) {
	...
	$search_cat = is_array($_REQUEST['search_cat'] ?? null) ? $_REQUEST['search_cat'] : [];
	$search_cat = array_values(array_filter(array_map('intval', $search_cat), fn($v) => $v > 0 || $v === -1));
	...
	<div class="form-group">
		<div class="row">
			<div class="col-sm-12">
				...
				<div class="dblock">
					<select data-placeholder="{$lang['addnews_cat_sel']}" name="search_cat[]" class="categoryselect" style="width:100%;max-width:24.3em;" multiple><option value="" {$c_all_s}>{$lang['edit_all']}</option><option value="-1" {$c_none_s}>{$lang['cat_in_none']}</option>{$category_list}</select>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="form-group">
		<div class="row">
			...
			<div class="col-sm-6">
				...
				<div style="width:100%">{$lang['edit_fdate']} <input data-rel="calendar" class="form-control" style="width:11.11em;" type="text" dir="auto" name="fromnewsdate" id="fromnewsdate" value="{$fromnewsdate}" autocomplete="off">
				{$lang['edit_tdate']} <input data-rel="calendar" class="form-control" style="width:11.11em;" type="text" dir="auto" name="tonewsdate" id="tonewsdate" value="{$tonewsdate}" autocomplete="off"></div>
			</div>
		</div>
	</div>
	<div class="form-group">
...
<div class="panel panel-default">
  ...
  <div class="panel-body">
	<div style="display: table;min-height:6.94em;width:100%;">
...
<div class="panel panel-default">
  <div class="panel-heading">
    ...
    <table class="table table-striped table-xs table-hover">
      <thead>
      <tr>
        <th class="hidden-xs hidden-sm" style="width: 4.17em;">&nbsp;</th>
        <th>{$lang['edit_title']}</th>
        <th class="hidden-xs text-center" style="width: 4.17em;"><i class="fa fa-eye tip" data-original-title="{$lang['st_views']}"></i></th>
        <th class="hidden-xs text-center" style="width: 4.17em;"><i class="fa fa-comment-o tip" data-original-title="{$lang['edit_com']}"></i></th>
        <th style="width:2.08em;text-align:center;">&nbsp;</th>
        <th class="hidden-xs text-center">{$lang['edit_cl']}</th>
        <th class="hidden-xs hidden-sm" style="max-width: 9.72em">{$lang['edit_autor']}</th>
        <th style="width: 2.78em;text-align:center;"><input type="checkbox" name="master_box" title="{$lang['edit_selall']}" onclick="javascript:ckeck_uncheck_all();" class="icheck"></th>
      </tr>
      </thead>
	  <tbody>
		{$entries}
...
HTML;
	...
	$xf_root = implode('', $xf_root);

	if( count($xfrubrics) ) {

		foreach( $xfrubrics as $key => $xfrubric) {
			$r_fields = DLEXFields::GETFieldByRubric($key);

			$tab_fields = [];
			foreach($r_fields as $value){
				if( isset($xfields['fields'][$value['name']]) ) {
					$tab_fields[] = $xfields['fields'][$value['name']];
				}
			}

			if( count($tab_fields) ) {
				$xf_tabs[] = "<li><a href=\"#xf_{$key}\" data-toggle=\"tab\"><i class=\"fa fa-pencil-square-o position-left\"></i> {$xfrubric['title']}</a></li>";
				$xf_tabs_body[] = "<div class=\"tab-pane\" id=\"xf_{$key}\"><div class=\"panel-body\">" . implode('', $tab_fields) . "</div></div>";
			}
		}

		$xf_tabs = implode('', $xf_tabs);
		$xf_tabs_body = implode('', $xf_tabs_body);
		
	} else { $xf_tabs = ''; $xf_tabs_body = '';}
...
function clearPoll(id) {
    ...
    DLEconfirmDelete( '{$lang['clear_poll_1']}', '{$lang['p_confirm']}', function () {
		...
		$.get("index.php?controller=ajax&mod=adminfunction", { id: id, action: 'clearpoll', user_hash: '{$dle_login_hash}' }, function(data){
...
function clearsubscribe(id) {
    ...
    DLEconfirmDelete( '{$lang['confirm_action']}', '{$lang['p_confirm']}', function () {
		...
		$.get("index.php?controller=ajax&mod=adminfunction", { id: id, action: 'clearsubscribenews', user_hash: '{$dle_login_hash}' }, function(data){
...
function MarkSpam(id, hash) {
    ...
    DLEconfirm( '{$lang['mark_spam']}', '{$lang['p_confirm']}', function () {
		...
		$.get("index.php?controller=ajax&mod=adminfunction", { id: id, action: 'newsspam', user_hash: hash }, function(data){
...
function save_edit_alert() {
	...
	$.post( "index.php?controller=ajax&mod=adminfunction", { 'id': '{$row['id']}', action: 'saveeditnews', user_hash: dle_login_hash }, function(data){
...
jQuery(function($){
   ...
   var max_textarea_height = 300 * getBaseSize();

	$('[data-toggle="tab"]').on('shown.bs.tab', function(e) {
		var id = $(e.target).attr("href");

		$(id).find(".editor-group textarea.classic").each(function () {
			this.style.height = "auto";
			if(this.scrollHeight > max_textarea_height ){
				this.style.height = max_textarea_height + "px";
		...
		if (typeof file_uploaders !== 'undefined') {
			for (var key in file_uploaders) {
				if (!file_uploaders.hasOwnProperty(key)) continue;

				var uploader = file_uploaders[key];
				if (!uploader || typeof uploader.refresh !== "function") continue;

				var container = uploader.settings.container;
				var containerElement = $(container);

				if (containerElement.closest(id).length > 0) {
					uploader.refresh();
				}
			}
		}
...
HTML;
    ...
    function preview(){ 
		...
		if(document.addnews.title.value == '') { 
			...
			if(width > (1024 * getBaseSize()) ) width = (1024 * getBaseSize());
			if(height > (650 * getBaseSize()) ) height = (650 * getBaseSize());
			...
			document.addnews.mod.value='preview';document.addnews.target='preview_iframe';document.addnews.submit();
    ...
    function sendNotice( id ){
		...
		b['{$lang['p_send']}'] = function() {
			if ( $('#dle-promt-text').val().length < 1) {
				...
				$('#dlepopup').remove();
				ShowLoading('');
				$.post('index.php?controller=ajax&mod=message', { id: id,  text: response, user_hash: '{$dle_login_hash}', allowdelete: \"no\" },
					function(data){
						HideLoading('');
		...
		$('body').append(\"<div id='dlepopup' class='dle-promt' title='{$lang['p_title']}' style='display:none'>{$lang['p_text']}<br><br><textarea dir='auto' name='dle-promt-text' id='dle-promt-text' class='classic' style='width:100%;' rows='6'></textarea></div>\");
    ...
    function confirmDelete(url, id){
		...
		var ww1 = 600 * getBaseSize();

		if(ww > ( $(window).width() * 0.95 ) )  { ww = $(window).width() * 0.95;  }
		if(ww1 > ( $(window).width() * 0.95 ) )  { ww1 = $(window).width() * 0.95;  }
			...
			click: function () {
				...
				bt['{$lang['p_send']}'] = function() {
								if ( $('#dle-promt-text').val().length < 1) {
									...
									$('#dlepopup').remove();
									ShowLoading('');
									$.post('index.php?controller=ajax&mod=message', { id: id,  text: response, user_hash: '{$dle_login_hash}' },
										function(data){
											HideLoading('');
				...
				$('body').append(\"<div id='dlepopup' title='{$lang['p_title']}' class='dle-promt' style='display:none'>{$lang['p_text']}<br><br><textarea dir='auto' name='dle-promt-text' id='dle-promt-text' class='classic' style='width:100%;' rows='6'></textarea></div>\");
	...
	function auto_keywords ( key, ai ) {
		...
		formData.delete('mod');
		formData.delete('meta_title');
		formData.delete('descr');
		formData.delete('keywords');

		ShowLoading('');

		$.post(\"index.php?controller=ajax&mod=keywords&user_hash={$dle_login_hash}\", formData, function(data){
			...
			if (key == 1) { $('#autodescr').val(data.content); }
			else { $('#keywords').tokenfield('setTokens', data.content);}
	...
	function find_relates ()
	{
		...
		$.post('index.php?controller=ajax&mod=find_relates', { title: title, id: '{$row['id']}', user_hash: '{$dle_login_hash}' }, function(data){
	...
	function find_related_ids ( id ){
		...
		$.post(\"index.php?controller=ajax&mod=adminfunction\", { action: 'relatedids', id: id, title: title, short_txt: short_txt, full_txt: full_txt, category: catselected, user_hash: '{$dle_login_hash}' }, function(data){
	...
	function xfimagedelete( xfname, xfvalue )
	{
		...
		DLEconfirmDelete( '{$lang['image_delete']}', '{$lang['p_info']}', function () {
			...
			$.post('index.php?controller=ajax&mod=upload', { subaction: 'deluploads', user_hash: '{$dle_login_hash}', news_id: '{$row['id']}', author: '{$author}', 'images[]' : xfvalue }, function(data){
	...
	function xffiledelete( xfname, xfvalue )
	{
		...
		DLEconfirmDelete( '{$lang['file_delete']}', '{$lang['p_info']}', function () {
			...
			$.post('index.php?controller=ajax&mod=upload', { subaction: 'deluploads', user_hash: '{$dle_login_hash}', news_id: '{$row['id']}', author: '{$author}', 'files[]' : xfvalue }, function(data){
	...
	function checkxf () {
		...
		$('[uid=\"essential\"]:not([data-hidden=\"true\"])').each(function(indx) {
			
			var el = $(this).find('[rel=\"essential\"]');

			var tempval =  StripHTML(el.val());
			tempval = tempval.trim();

			if (el.is('select')) {
				var selectedOption = el.find('option:selected');
				var tempval = selectedOption.text().trim();
			}
			...
			if(tempval.length < 1) {
				...
				if( el.data('alert') ) {
					...
					alert_text = alert_text.replace(/{field}/g, el.data('alert') );
...
{$save_edit_alert}
		...
		$( '#new_author' ).autocomplete({
			source: function( request, response ) {
				$.getJSON( 'index.php?controller=ajax&mod=find_tags&user_hash={$dle_login_hash}&mode=authors', {
					term: request.term
				}, response );
			},
			search: function() {
		...
		$('#tags').tokenfield({
		  autocomplete: {
		    source: 'index.php?controller=ajax&mod=find_tags&user_hash={$dle_login_hash}',
			minLength: 3,
		    delay: 500
		  },
		...
		$('[data-rel=links]').tokenfield({
		  autocomplete: {
		    source: 'index.php?controller=ajax&mod=find_tags&user_hash={$dle_login_hash}&mode=xfield',
			minLength: 3,
		    delay: 500
		  },
		  createTokensOnBlur:true
		...
		if( $('#category').hasClass('categoryselect') && $('#category_chosen').length ) {
			...
			var allow_cat_sort = '{$config['allow_cat_sort']}';
			
			if(allow_cat_sort == '1') {

				var cat_sort = new Sortable(document.querySelector('#category + .chosen-container ul.chosen-choices'), {
					group: \"catselect\",
					animation: 150,
					handle: '.search-choice',
					onSort: function (evt) {
						var cat_sorted = cat_sort.toArray();
						cat_sorted = cat_sorted.filter(item => item !== '2no');
						category_results = cat_sorted;
						$('#category_custom_sort').val(cat_sorted.join('::'));
						$('#category').trigger('chosen:close');
					}
	...
	if( $member_id['user_group'] < 3 ) {
		...
		$author_info = "<input type=\"text\" dir=\"auto\" name=\"new_author\" id=\"new_author\" class=\"form-control position-left\" style=\"width: 11.11em;\" value=\"{$row['autor']}\"><input type=\"hidden\" name=\"old_author\" value=\"{$row['autor']}\" />";
...
HTML;
	...
	if($config['enable_ai'] AND in_array($member_id['user_group'], explode(',', trim($config['ai_groups'])) ) ) {
		...
		$ai_metatags = <<<HTML
<div class="mt-10">
	<input type="button" onclick="auto_keywords(1, 1); return false;" class="btn bg-primary-600 btn-sm btn-raised position-left" value="{$lang['btn_descr_ai']}">
	<input type="button" onclick="auto_keywords(2, 1); return false;" class="btn bg-primary-600 btn-sm btn-raised" value="{$lang['btn_keyword_ai']}">
</div>
HTML;	
                     ...
                     <div class="tab-pane active" id="tabhome">
						<div class="panel-body">
							...
							<div class="form-group">
							  ...
							  <div class="col-sm-10">
								<input type="text" dir="auto" name="newdate" data-rel="calendar" class="form-control position-left" style="width: 11.11em;" value="{$row['date']}" autocomplete="off"><label class="checkbox-inline"><input class="icheck" type="checkbox" name="allow_now" id="allow_now" value="yes">{$lang['edit_jdate']}</label>
							  </div>
							 </div>
							...
							<div class="form-group">
							  ...
							  <div class="col-sm-10">
								<select data-placeholder="{$lang['addnews_cat_sel']}" name="category[]" id="category" onchange="onCategoryChange(this)" {$category_multiple} style="width:100%;max-width:24.3em;">{$categories_list}</select><input type="hidden" name="category_custom_sort" id="category_custom_sort" value="">
							  </div>
							 </div>
HTML;
						...
						<div class="panel-body">
						 ...
						 <div class="panel-body">
							...
							<div class="form-group">
							  ...
							  <div class="col-md-10 col-sm-9">
								<textarea dir="auto" class="tags" dir="auto" name="keywords" id='keywords'>{$row['keywords']}</textarea>
								<div class="mt-10">
...
HTML;
	...
	$allow_comm = (int)($_POST['allow_comm'] ?? 0);
	$allow_main = (int)($_POST['allow_main'] ?? 0);
	$approve = (int)($_POST['approve'] ?? 0);
	$allow_rating = (int)($_POST['allow_rating'] ?? 0);
	$news_fixed = (int)($_POST['news_fixed'] ?? 0);
	$view_edit = (int)($_POST['view_edit'] ?? 0);
	$category = filter_input(INPUT_POST, 'category', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY) ?: [];
	$disable_index = (int)($_POST['disable_index'] ?? 0);
	$disable_search = (int)($_POST['disable_search'] ?? 0);
	$need_pass = (int)($_POST['need_pass'] ?? 0);
	$ifdelete = $_REQUEST['ifdelete'] ?? '';
	$news_fixed = $_REQUEST['news_fixed'] ?? '';

	$allow_rss = (int)($_POST['allow_rss'] ?? 0);
	$allow_rss_dzen = (int)($_POST['allow_rss_dzen'] ?? 0);

	if ($config['allow_cat_sort'] && count($parts = explode('::', $_POST['category_custom_sort'] ?? '')) > 1) {
		$category = $parts;
		unset($parts);
	}
	...
	foreach ( $category_list as $selected ) {
		...
		if($member_id['user_group'] > 2 AND $selected) {
			if(isset($cat_info[$selected]['enable_dzen']) AND !$cat_info[$selected]['enable_dzen']) $disable_rss_dzen ++;
		}
	...
	if (trim($_POST['vote_title'] ?? '')) {
		...
		$allow_m_vote = (int)($_POST['allow_m_vote'] ?? 0);
		$closed_vote = ($_POST['closed_vote'] ?? 0) ? 1 : 0;
		$closed_time = $closed_vote ? $_TIME : '';
	} else {
		$add_vote = 0;
	}

	$related_ids = implode(',', array_filter(array_map('intval', explode(',', $_POST['related_ids'] ?? '')), fn($id) => $id > 0));
	...
	if( $member_id['user_group'] < 3 AND $ifdelete != "yes" ) {
		...
		$allowed_country = $db->safesql(strip_tags(trim($_POST['allowed_country'] ?? '')));
		$not_allowed_country = $db->safesql(strip_tags(trim($_POST['not_allowed_country'] ?? '')));
		
		$group_regel = "access='{$group_regel}',";
		$allowed_country = ",allowed_country='{$allowed_country}',";
		$not_allowed_country = "not_allowed_country='{$not_allowed_country}'";
	...
	$movecat = filter_input(INPUT_POST, 'movecat', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY) ?: [];

	if (!count($movecat)) $movecat[] = 0;

	$movecat_list = $db->safesql(implode(',', array_map('intval', $movecat)));
	
	$expires_date = $_POST['expires'] ?? '';
	$expires_action = $_POST['expires_action'] ?? '';
	$expires_alt = $_POST['expires_alt'] ?? '';

	if (!$add_vote && $expires_action == 6) {
		$expires_date = '';
		$expires_action = 0;
	}

	if (($expires_date . $expires_action . $movecat_list) != $expires_alt) {
		
		if (trim($expires_date) != '') {
			if ((($expires = strtotime($expires_date)) === -1) || !$expires) {
	...
	if( $ifdelete != "yes" ) {
		...
		$parsed_fields = DLEXFields::Parse($item_db[9]);

		if (DLEXFields::$error) {
			msg("error", "error", DLEXFields::$error, "javascript:history.go(-1)");
		}
	...
	if( isset($item_db[0]) AND $item_db[0] ) {
		...
		if( $have_perm ) {
			...
			if( $ifdelete != "yes" ) {
				...
				if ($item_db[6]) $db->query( "UPDATE " . PREFIX . "_post_extras SET allow_rate='{$allow_rating}', votes='{$add_vote}', disable_index='{$disable_index}', related_ids='{$related_ids}', {$group_regel} editdate='{$added_time}', editor='{$member_id['name']}', reason='{$editreason}', view_edit='{$view_edit}', disable_search='{$disable_search}', need_pass='{$need_pass}', allow_rss='{$allow_rss}', allow_rss_dzen='{$allow_rss_dzen}', edited_now=''{$allowed_country}{$not_allowed_country} WHERE news_id='{$item_db[0]}'" );
				...
				if ( $expires_change ) {
					...
					$expires_action = (int)$expires_action;
