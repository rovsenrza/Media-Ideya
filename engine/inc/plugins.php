...
function makeDropDown($options, $name, $selected) {
	...
	$output = "<select class=\"uniform\" name=\"{$name}\" style=\"min-width:6.94em;\">\r\n";
...
if( $_GET['action'] == "on" OR $_GET['action'] == "off" ) {
	...
	if( !check_referer($_SERVER['SCRIPT_NAME']."?mod=plugins") ) {
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="form-group">
						<div class="col-sm-12">
						  <input type="file" name="pluginfile" accept="text/xml,application/zip" class="icheck">
						</div>
				...
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							...
							<input type="text" class="form-control position-left" name="ftp[port]" style="width:3.125em" value="21">
						</div>
					</div>
					<div class="form-group">
...
$js_func = <<<HTML
	...
	function PluginUpdateFromURL(id, url, version){
		...
		$.ajax({
			url: "index.php?controller=ajax&mod=plugins",
	...
	if( $_REQUEST['action'] == "add" ) {
		...
		while ( $row = $db->get_row() ) {
			...
			if( $row['plugin_id'] AND $row['type'] == "mysql") {
				...
				$row['area'] = "<pre class=\"language-sql\" data-prismjs-copy=\"<svg xmlns='http://www.w3.org/2000/svg' width='0.9em' height='0.9em' viewBox='0 0 15 15'><path fill='none' stroke='currentcolor' d='M11 1.5h2.5v12a1 1 0 0 1-1 1h-10a1 1 0 0 1-1-1v-12H4m1 7l2 2l3.5-4m-6-6h6v2a1 1 0 0 1-1 1h-4a1 1 0 0 1-1-1z' stroke-width='1'></path></svg> {$lang['pre_copy']}\" data-prismjs-copy-success=\"<svg xmlns='http://www.w3.org/2000/svg' width='1em' height='1em' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><path d='M18 6 7 17l-5-5'/><path d='m22 10-7.5 7.5L13 16'/></svg> {$lang['pre_copied']}\"><code>".$row['area']."</code></pre>";
			} elseif( $row['plugin_id'] AND $row['type'] == "file" ) {
				$error_ids[] = $row['action_id'];
				$area = $lang['plugins_filename'];
				$row['error'] = $lang['plugins_errors_4']."<pre class=\"language-php\" data-prismjs-copy=\"<svg xmlns='http://www.w3.org/2000/svg' width='0.9em' height='0.9em' viewBox='0 0 15 15'><path fill='none' stroke='currentcolor' d='M11 1.5h2.5v12a1 1 0 0 1-1 1h-10a1 1 0 0 1-1-1v-12H4m1 7l2 2l3.5-4m-6-6h6v2a1 1 0 0 1-1 1h-4a1 1 0 0 1-1-1z' stroke-width='1'></path></svg> {$lang['pre_copy']}\" data-prismjs-copy-success=\"<svg xmlns='http://www.w3.org/2000/svg' width='1em' height='1em' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><path d='M18 6 7 17l-5-5'/><path d='m22 10-7.5 7.5L13 16'/></svg> {$lang['pre_copied']}\"><code>".$row['error']."</code></pre>";
			} elseif( $row['plugin_id'] AND $row['type'] == "upload" ) {
				$area = $lang['plugins_filename'];
				$row['error'] = "<pre class=\"language-php\" data-prismjs-copy=\"<svg xmlns='http://www.w3.org/2000/svg' width='0.9em' height='0.9em' viewBox='0 0 15 15'><path fill='none' stroke='currentcolor' d='M11 1.5h2.5v12a1 1 0 0 1-1 1h-10a1 1 0 0 1-1-1v-12H4m1 7l2 2l3.5-4m-6-6h6v2a1 1 0 0 1-1 1h-4a1 1 0 0 1-1-1z' stroke-width='1'></path></svg> {$lang['pre_copy']}\" data-prismjs-copy-success=\"<svg xmlns='http://www.w3.org/2000/svg' width='1em' height='1em' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'><path d='M18 6 7 17l-5-5'/><path d='m22 10-7.5 7.5L13 16'/></svg> {$lang['pre_copied']}\"><code>".$row['error']."</code></pre>";
			} else {
	...
	if(count($files)) {
		...
		foreach( $files as $key => $value ) {
			...
			foreach ($value as $value2) {
				...
				$x .=  "<div id=\"actionset_{$total_action}\" class=\"alert pb-5{$class}\" style=\"position:relative\"><div style=\"position: absolute;right: 1.39em;bottom: .1em;z-index: 2;\"><a onclick=\"AddAction('{$total_files}', '{$total_action}'); return false;\"><i class=\"fa fa-plus text-success\"></i></a></div><fieldset>";
				...
				$x .=  "<div class=\"col-sm-10\">";
				$x .=  "<input type=\"text\" class=\"form-control position-left\" name=\"filedleversion[{$total_files}][{$total_action}]\" value=\"{$value2['filedleversion']}\" style=\"width:6.94em;\">";
				$x .=  $filedleversion;
				$x .=  "</div>";
				$x .=  "</div>";
...
<script src="public/editor/ace/code_editor.js"></script>
<script src="public/editor/ace/langs/{$lang['language_code']}.js"></script>
		...
		<div class="panel panel-default">
            ...
            <div class="panel-tab-content tab-content">
                ...
                <div class="tab-pane" id="notinfo">
					<div class="panel-body">
						<textarea dir="auto" name="notice" class="classic" style="width:100%;" rows="9" placeholder="{$lang['main_no_p_notice']}">{$row['notice']}</textarea>
...
function AddAction(fileid, placeid){
	...
	var x =  "<div id=\"actionset_" + actions + "\" class=\"alert border-success pb-5\"><div style=\"position: absolute;right: 1.39em;bottom: .1em;z-index: 2;\"><a onclick=\"AddAction('" + fileid + "', '" + actions + "'); return false;\"><i class=\"fa fa-plus text-success\"></i></a></div><fieldset>";
	x += "<legend><span class=\"position-left\">{$lang['vote_action']}</span>";
...
function onActionChange(obj, fileid, actionid) {
	...
	x +=  "<label class=\"control-label col-sm-2\">{$lang['plugins_dlever']}</label>";
	x +=  "<div class=\"col-sm-10\">";
	x +=  "<input type=\"text\" class=\"form-control position-left\" name=\"filedleversion[" + fileid + "][" + actionid + "]\" value=\"\" style=\"width:6.94em;\">";
	x +=  "<select class=\"uniform\" name=\"fileversioncompare[" + fileid + "][" + actionid + "]\" style=\"min-width:6.94em;\">";
...
HTML;
	...
	while ( $row = $db->get_row() ) {
		...
		if( $row['mnotice'] AND $row['notice']) {
			...
			$row['notice'] = "<div class=\"alert alert-info p-5\" style=\"max-height:13.89em;overflow:auto;cursor:auto;\">".stripslashes($row['notice'])."</div>";
...
HTML;
		...
		$entries .= "
			<tr class=\"drag-bg allow-drag\" data-id=\"{$row['id']}\">
				<td class=\"dd-handles\" style=\"width: 2.78em;\"></td>
				<td class=\"cursor-pointer text-center\" style=\"width: 6.25em;\" onclick=\"document.location = '?mod=plugins&action=edit&id={$row['id']}'; return false;\"><img src=\"{$row['icon']}\" class=\"img-lg section_icon\"></td>
				<td class=\"cursor-pointer\" onclick=\"document.location = '?mod=plugins&action=edit&id={$row['id']}'; return false;\"><h6 class=\"media-heading text-semibold\">{$row['name']}{$row['version']}{$status}{$plugin_error}</h6><span class=\"text-muted text-size-small\">{$row['description']}</span><span id=\"upgrade{$row['id']}\" class=\"label label-success position-right text-size-small\"></span>{$row['notice']}</td>
	...
	if( !$entries ) {
		...
		$entries = "<tbody id=\"storagelist\">".$entries."</tbody>";
	}
...
<input type="hidden" name="mod" value="plugins">
	<div class="panel panel-default">
	  <div class="panel-heading">
		...
		<div class="heading-elements">
			<div class="form-group has-feedback" style="width:17.36em;">
	...
	jQuery(function($){
		...
		if( document.getElementById('storagelist') ) {
			var storage_sort = new Sortable(document.getElementById('storagelist'), {
				group: "xfield",
				animation: 150,
				ghostClass: 'drop-bg',
				handle: '.dd-handles',
				...
				onSort: function (evt) {
					...
					$.post('index.php?controller=ajax&mod=adminfunction', {'action': 'pluginsort', 'list': window.JSON.stringify(storage_sort.toArray()), user_hash: '{$dle_login_hash}'}, function(data){
