...
if( isset($_GET['douser']) AND isset($_GET['lostid']) AND intval( $_GET['douser'] ) AND $_GET['lostid'] ) {
	...
	if( preg_match( "/[\||\'|\<|\>|\[|\]|\"|\!|\?|\$|\/|\\\|\&\~\*\{\+]/", $_POST['lostname'] ) OR !trim($_POST['lostname'])) {
	...
	} elseif( !isset($_POST['sec_code']) OR !isset($_SESSION['sec_code_session']) OR $_POST['sec_code'] != $_SESSION['sec_code_session'] OR !$_SESSION['sec_code_session'] ) {
		...
		if( isset($row['user_id']) AND $row['user_id'] AND $user_group[$row['user_group']]['allow_admin'] ) {
			...
			$lostid = UniqIDReal(40);

			if ( !$lostid ) die ("Secure Hash Algorithm disabled by Hosting");
	...
	if ( $config['allow_recaptcha'] ) {
		...
		$text .= "<div class=\"form-group\"><a onclick=\"reload(); return false;\" href=\"#\" title=\"{$lang['reload_code']}\"><span id=\"dle-captcha\"><img src=\"index.php?controller=antibot\" alt=\"{$lang['reload_code']}\" style=\"width: 130px;height: 46px;\"></span></a>&nbsp;<input placeholder=\"{$lang['repead_code']}\" type=\"text\" dir=\"auto\" name=\"sec_code\" id=\"sec_code\" class=\"classic\" style=\"height: 46px;vertical-align: middle;\" required></div>";
