...
elseif( isset($_GET['do']) AND $_GET['do'] == 'cat' AND $category_id) $config['home_title'] = stripslashes( $cat_info[$category_id]['name'] ) . " - " . $config['home_title'];

$self_url = htmlspecialchars(((isSSL() || $config['only_ssl']) ? 'https' : 'http') . '://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], ENT_QUOTES, "utf-8");
