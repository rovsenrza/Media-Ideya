...
- Statue (`47:715`): fixed to the CTA's right edge at `201×395`. Figma applies a horizontal mirror (`rotate(180deg) scaleY(-1)`), so the face enters from the right edge and looks left. Reuse the existing original `images/pages/cases/head.png` asset with `object-position: right center` and `scaleX(-1)`; exported MCP raster returned an empty placeholder.
